...
function SkillGapRadar({ data }) {
  // Ensure data is always at least an empty array
  const chartData = data || [];

  // Only render when there's valid data
  if (!chartData.length) {
  ...
  return (
    <RadarChart
      ...
      height={400}
      data={chartData}
    // optionally specify radiusAxisId if you have multiple radius axes
    >
      <PolarGrid />
                  ...
                  <div className="space-y-3">
                    ...
                    {currentIDP.goals.slice(0, 3).map((goal) => (
                      <div key={goal.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        ...
                        <Badge
                          variant="outline"
                          className={`text-xs ${goal.status === 'completed' ? 'border-green-200 text-green-700' :
              ...
              <div className="space-y-3">
                {recentNotifications.map((notification) => (
                  <div key={notification.id} className="p-3 bg-gray-50 rounded-lg">
                    <div className="flex items-start space-x-3">
                      <div className={`mt-1 w-2 h-2 rounded-full ${notification.priority === 'high' ? 'bg-red-500' :
